...
* @Last Modified time: 2016-04-25 14:02:01
*/
...
module.exports = Generator.Base.extend({
	...
	prompting: function(){
		...
		this.prompt(Q, function(answer){
			...
			fs.mkdirSync(me.answer.appName);
	...
	_installDependencies: function(){
		...
		if (pkgList && pkgList.length){	
			const params = packageInstaller === 'npm' ? ['install', ...pkgList, '-S'] : ['add', ...pkgList, '-S'];
			me.spawnCommandSync(packageInstaller, params);
		}

		//install DevDependencies
		me._getBabelDev(DevDependencies);
		const params = packageInstaller === 'npm' ? ['install', ...DevDependencies, '-D'] : ['add', ...DevDependencies, '-D'];
		me.spawnCommandSync(packageInstaller, params);
	...
	_generatorDirectory(data){
		...
		const destRoot   = me.destinationRoot(`./${me.answer.appName}`);
